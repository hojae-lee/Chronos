                ...
                >
                  {msg.role === 'user' ? (
                    ...
                    <div className="bg-background-elevated rounded-2xl rounded-tl-sm overflow-hidden max-w-[88%]">
                      <p className="text-sm text-text-primary px-4 pt-3 pb-2 leading-relaxed">{msg.text}</p>
                      <ul className="pb-2">
                        {msg.events.map((e) => {
                            ...
                            <li key={e.id}>
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className={['text-xs font-medium text-text-primary truncate', e.status === 'cancelled' ? 'line-through opacity-50' : ''].join(' ')}>
                                    {e.title}
                                  </p>
                                  <p className="text-[11px] text-text-disabled">{dateLabel} · {timeLabel}</p>
                                </div>
                              </button>
                            </li>
                          )
                    ...
                    <div className="bg-background-elevated rounded-2xl rounded-tl-sm px-4 py-3 max-w-[88%]">
                      <p className="text-sm text-text-primary leading-relaxed mb-2">{msg.text}</p>
                      <button
                        onClick={() => props.onOpenRetrospective?.(msg.retroYear!, msg.retroMonth!)}
                    ...
                    <div className="bg-background-elevated rounded-2xl rounded-tl-sm px-4 py-2.5 max-w-[88%] text-sm text-text-primary leading-relaxed">
                      <ReactMarkdown
                        components={{
                          ...
                          strong: ({ children }) => <strong className="text-brand-primary font-semibold">{children}</strong>,
                          em: ({ children }) => <em className="not-italic text-text-secondary">{children}</em>,
                          ...
                          h2: ({ children }) => <p className="font-bold text-text-primary mb-1">{children}</p>,
                          h3: ({ children }) => <p className="font-semibold text-text-primary mb-0.5">{children}</p>,
                        }}
                      >
                        {msg.text}
                      </ReactMarkdown>
